...
const accessToken =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjoiMTIzIiwic2NvcGUiOiJSRUFEIiwiaWF0IjoxNTM3MzEwMzc5LCJleHAiOjE1MzczMTM5NzksInN1YiI6IjEifQ.JInJ0i_twklKodOxv0Rwae1j-C7XnraE4ux-BAQgDm4";
const code =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjoiMTIzIiwic2NvcGUiOiJSRUFEIiwiaWF0IjoxNTM3MzEyOTU5LCJleHAiOjE1MzczMTY1NTksInN1YiI6IjEifQ.qAbUxBD5GuZsnW4JVXAF-vBGXEtV-hJ51ySqmmq9dvk";
const refreshToken =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjoiMTIzIiwic2NvcGUiOiJSRUFEIiwiaWF0IjoxNTM3MzEwNjQ0LCJleHAiOjE1MzczMTQyNDQsInN1YiI6IjEifQ.iRt50FDe7YSdzRy_f8RNV0aaxcQRZg8GjB6L2aMBPh8";
...
describe("getClient", () => {
  test("Should return client when valid parameters are passed", () => {
    return expect(
      model.getClient(client[0].id, client[0].clientSecret)
    ).resolves.toBeTruthy();
  });

  test("Should return client with values matching the parameters passed", async () => {
    return expect(
      await model.getClient(client[0].id, client[0].clientSecret)
    ).toEqual(
...
describe("getAuthorizationCode", () => {
  test("Should return authorizationCode object when valid code is passed", () => {
    return expect(
      model.getAuthorizationCode(authorizationCodes[1].code)
    ).resolves.toBeTruthy();
  });
  ...
  test("Should not return an authorizationCode when invalid info is passed", () => {
    return expect(
      model.getAuthorizationCode("dsfhkjsdfgh7923rygu2hr9eyrduf")
    ).rejects.toBeTruthy();
  });
});
  ...
  test("Should add Token to the db matching parameters passed", async () => {
    ...
    return expect(res).toEqual(
      expect.objectContaining({
        accessToken,
        refreshToken,
        client: expect.objectContaining({ id: client[0].id }),
...
describe("getAccessToken", () => {
  test("Should return token for valid access token passed", async () => {
    return expect(await model.getAccessToken(accessToken)).toEqual(
  ...
  test("Should not return token for invalid access token", () => {
    return expect(
      model.getAccessToken("jsdfhksjdhflyr432hw89y4jn")
    ).rejects.toBeTruthy();
  });
...
describe("revokeAuthorizationCode", () => {
  test("Should delete authorizationCode with matching parameters", async () => {
    let deletedRes = await model.revokeAuthorizationCode(
      authorizationCodes[1].code
    );
  ...
  test("Should not delete authorizationCode when invalid parameters are passed", () => {
    return expect(
      model.revokeAuthorizationCode("jdfhksdhfkjdsfhlajds")
    ).rejects.toBeTruthy();
  });
});
