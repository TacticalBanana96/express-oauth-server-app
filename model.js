...
function generateAuthorizationCode(client, user, scope) {
  let code = jwt.sign({ user: user.id, scope }, secretKey, {
    expiresIn: 3600,
    subject: client.id
  });
  return code;
...
async function getAuthorizationCode(authorizationCode) {
  ...
  return Promise.resolve({
    ...
    user: { id: user.userId }
    //client: new Client({id: client.clientId,  clientSecret: client.secret,redirectUri: client.redirectUri,grants: client.grants}),
    //user: new User({id: user.userId, username: user.username, password: user.password})
  });
}
...
async function saveToken(token, client, user) {
  ...
  return Promise.resolve({
    ...
    // client: new Client({id: accessToken.clientId,  clientSecret: client.secret,redirectUri: client.redirectUri,grants: client.grants}),
    //user: new User({id: accessToken.userId, username: user.username, password: user.password})
  });
}
...
async function saveAuthorizationCode(code, client, user) {
  ...
  return await authCode.save().then(authCode => {
    return Promise.resolve({
      ...
      //client: new Client({id: authCode.clientId,  clientSecret: client.secret,redirectUri: client.redirectUri,grants: client.grants}),
      //user: new User({id: authCode.userId, username: user.username, password: user.password})
    });
  });
}
